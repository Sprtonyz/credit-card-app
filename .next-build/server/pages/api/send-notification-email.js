...
;// CONCATENATED MODULE: ./pages/api/send-notification-email.js
...
async function handler(req, res) {
    ...
    const recipients = parseRecipients(payload.to || process.env.EMAIL_TEST_RECIPIENT || DEFAULT_TEST_RECIPIENT);
    const reports = Array.isArray(payload.reports) && payload.reports.length > 0 ? payload.reports : [
        {
    ...
    if (recipients.length > 50) {
        return res.status(400).json({
            error: "Too many recipients for one email."
        });
    }
    const sent = [];
